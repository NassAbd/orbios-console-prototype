        ...
        try:
            for task in task_queue:
                ...
                if task.progress >= 100.0:
                    ...
                    add_message("GS", f"Task {task.task_id} complete. Satellite idle.", "INFO")
                    continue
                ...
                if task.progress < 25:
                    ...
                    if task.stage != TaskStage.IMAGING_ACQUISITION:
                        ...
                        add_message("SAT", f"Initiating high-res imaging for {task.task_id}", "INFO")
                    ...
                    if task.stage != TaskStage.AI_INFERENCE:
                        ...
                        add_message("AI", f"OrbiOS AI Engine processing imagery for {task.task_id}", "WARN")
                else:
                    if task.stage != TaskStage.GS_REPORTING:
                        ...
                        add_message("GS", f"Downlink acquired. Analysis confirmed wildfire at {task.task_id}", "CRIT")
...
def get_telemetry() -> Telemetry:
    ...
    # Fake higher load if AI is processing
    is_ai_active = any(s["status"] == SatelliteStatus.AI_PROCESSING for s in satellite_dynamic_states.values())
    cpu_adj = random.uniform(15, 30) if is_ai_active else 0
    ...
    return Telemetry(
        ...
        container_status="INFERENCE_ACTIVE" if is_ai_active else "HEALTHY"
    )
...
def trigger_spike():
    ...
    task_id = f"MISSION-{len(task_queue)+1}"
    
    new_task = MissionTask(
        task_id=task_id,
        description=f"Anomaly at {fz_id}",
        priority=1,
        status="RUNNING",
    ...
    if assigned_id:
        satellite_dynamic_states[assigned_id] = {"status": SatelliteStatus.IDLE, "current_task_id": task_id}
        add_message("GS", f"Uplink: Mission {task_id} assigned to SAT-{assigned_id}", "INFO")
